...
| `tracker_version` | Logger sürümü (**v2**). |
...
| `git_commit` | Klasör git deposuysa kısa commit, değilse `null`. |
| `code_fingerprint` | `live_tracker.py` + `tracker_v2.py` içeriğinin sha256'sının ilk 16 hanesi. **Kod değişirse değişir** — farklı kodların verisini ayırmak için bunu kullan. |
